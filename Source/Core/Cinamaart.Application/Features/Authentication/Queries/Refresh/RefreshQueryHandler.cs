...
namespace Cinamaart.Application.Features.Authentication.Queries.Refresh
{
    public class RefreshQueryHandler(
        ...
        ) : IRequestHandler<RefreshQuery, Result<AuthenticationResultDTO>>
    {
        public async Task<Result<AuthenticationResultDTO>> Handle(RefreshQuery request, CancellationToken cancellationToken)
        {
            try
            {
